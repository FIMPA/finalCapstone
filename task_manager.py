"""
In this project, an existing file called task_manager.py, together with the
supporting text files user.txt and tasks.txt will be used. It is about 
modifying task_manager.py to extend its functionality. 
 
The main body of the code must have functionality for registering a user, adding
a task, viewing all tasks, viewing the current user's tasks, generate reports, 
and display statistics.

As there is so much functionality needed to complete, functions will be created 
using the principle of abstraction in order to facilitate the reading of the 
main body of the loop. 

They are following functions:
- reg_user (a function that is called when the user selects ‘r’ to register
a user, making sure that usernames are not duplicated when adding a new user 
to user.txt). 
- add_task (a function called when a user selects ‘a’ to add a newtask).
- view_all (a function called when users type ‘va’ to view all the tasks 
listed in ‘tasks.txt’.
- view_mine (a function called when users type ‘vm’ to view all the tasks that 
have been assigned to them.
When the user selects ‘vm’, all tasks should be displayed making sure that each 
task is displayed with a corresponding number that can be used to identify 
the task. 'vm' should allow the user to select either a specific task (by 
entering a number) or input ‘-1’ to return to the main menu. Moreover, If the 
user selects a specific task, they should be able to choose to either mark the 
task as complete or edit the task.
If the user chooses to mark a task as complete, the ‘Yes’/’No’ value that 
describes whether the task has been completed or not should be changed to 
‘Yes’.
If the user chooses to edit a task, the username of the person to whom the task 
is assigned or the due date of the task can be edited. The task can only be 
edited if it has not yet been completed.
- An option to generate reports, and another to display statistics will be 
added to the main menu of the application.

The menu for the admin user should finally look something like this:
...
- Two text files, called task_overview.txt and user_overview.txt, should be 
generated when the user chooses to generate reports.
- Finally, the menu option that allows the admin to display statistics will
generate the reports that will be read from tasks.txt and user.txt and displayed
on the screen in a user-friendly manner. If these text files do not exist
(because the user has not selected to generate them yet), first call the code
...
def load_tasks():
    '''Do load tasks from 'tasks.txt' file and return a list of tasks.
        That is, a list of dictionaries representing tasks.
        Contains username, title, description, due_date, assigned_date and
        completion status.
    '''
    # Need to create tasks.txt if it does not exist.
    if not os.path.exists("tasks.txt"):
        with open("tasks.txt", "w") as default_file:
            pass
    # Do read the task data from tasks.txt.        
    with open("tasks.txt", 'r') as task_file:
        task_data = task_file.read().split("\n")
        task_data = [t for t in task_data if t != ""]

    task_list = []
    # Do convert the data into dictionary format.
...
#~~~~~~~~~~Login Section~~~~~~~~~~
'''This program reads usernames and password from the user.txt file to 
...
def load_users():
    '''Loads users from 'user.txt' file and return a dictionary mapping
        ...
        It returns the dictionary mapping usernames to passwords.
    '''
    if not os.path.exists("user.txt"):
        with open("user.txt", "w") as default_file:
            default_file.write("admin;password")

    # Reads user data from user.txt.
...
# This function allows to register a new user by asking for a username and password.
...
# This function will add a new task which will be updated in the 'tasks.txt' file.
...
# This function allows to view all the tasks which are displayed in the task list.   
...
def display_statistics(username_password, task_list):
    ...
    if not os.path.exists("tasks.txt") or not os.path.exists("user.txt"):
        # If required files do not exist, they will get generated.
        load_tasks()
        load_users()
...
# This function allows to generate the reports.       
